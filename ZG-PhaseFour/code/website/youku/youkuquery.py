...
# @note：
################################################################################################################
class YoukuS2Query(SiteS2Query):
    QUERY_TEMPLATE = 'http://www.soku.com/search_video/q_{key}_limitdate_7?site=14&_lg=10&orderby=2'
    QUERY_TEMPLATE2 = 'http://www.soku.com/search_video/q_{key}_limitdate_7?site=14&orderby=2&_lg=10&page={pageno}'
    ...
    # @param： none
    # @return：none
    # @note：youkuS2Query，初始化内部变量
    ################################################################################################################
    def __init__(self):
        # 使用该URL识别回传S2查询结果的类，推荐使用主站URL
        SiteS2Query.__init__(self)
        self.fakeoriginalurl = 'http://www.youku.com/'
    ################################################################################################################
    # @functions：query
    ...
    def query(self, info):
        ...
        YoukuS2Query.QUERY_TEMPLATE = 'http://www.soku.com/search_video/q_{key}_limitdate_{querydays}?site=14&_lg=10&orderby=2&page={page}'
        keyvalue = Common.urlenc(info)
        urls = [YoukuS2Query.QUERY_TEMPLATE.format(key=keyvalue,querydays=self.querylastdays,page=0)]
    ...
    def process(self, params):
        if params.step == YoukuS2Query.S2QUERY_FIRST_PAGE:
            self.step1(params)
        elif params.step == YoukuS2Query.S2QUERY_EACH_PAGE:
            self.step2(params)
    #----------------------------------------------------------------------
    def step1(self,params):
        ...
        if not nexted or pages_num == self.maxpages or (nexted and pages_num < max(pages_num, 10) ):
            for page in range(1,pages_num+1):
                querylist.append(YoukuS2Query.QUERY_TEMPLATE.format(key=q, querydays=self.querylastdays, page=page))
            self.__storeqeuryurllist__(querylist, self.S2QUERY_EACH_PAGE, {'query': q})
            return
        querylist.append(YoukuS2Query.QUERY_TEMPLATE.format(key=q, querydays=self.querylastdays, page=pages_num))

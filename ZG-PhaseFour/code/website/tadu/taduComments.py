...
##############################################################################################
class TaDuComments(SiteComments):
    COMMENT_URL ='http://www.tadu.com/book/comment-{articleId}-2-{page}'
    PAGE_SIZE = 30
    STEP_1 = None
    STEP_2 = 2
    STEP_3 = 3
    ...
    def process(self, params):
        ...
        try:
            if params.step is TaDuComments.STEP_1:
                #Step1: 通过得到docurl，得到获取评论的首页url参数。
                articleId = self.r.parse('http://\w+\.tadu\.com/\w+/(\d+).*', params.originalurl)[0]

                # 取得评论的url列表
                comments_url = TaDuComments.COMMENT_URL.format (articleId = articleId,page = 1)
                self.storeurl(comments_url, params.originalurl, TaDuComments.STEP_2, {'articleId': articleId})

            elif params.step == TaDuComments.STEP_2:
                # 获得评论参数
